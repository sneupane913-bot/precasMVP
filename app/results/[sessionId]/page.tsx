...
export default async function ResultsPage({ params }: { params: { sessionId: string } }) {
  const session = await store.get(params.sessionId);
  if (!session) notFound();
          ...
          <section className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[
              ...
              <div key={label as string} className="rounded-xl border border-slate-200 bg-white p-4">
                <p className="text-2xl font-black text-ink">{value as number}%</p>
                <p className="text-sm font-semibold text-ink">{label as string}</p>
                <p className="mt-0.5 text-xs leading-snug text-slate-500">{hint as string}</p>
              </div>
            ))}
          </section>
        )}
