...
const STRUCTURE = [
  { n: 1, label: 'Point', hint: 'Answer the question in one sentence.' },
  { n: 2, label: 'Reason', hint: 'Say why, in your own words.' },
  { n: 3, label: 'Example', hint: 'Give one real detail from your life.' },
  { n: 4, label: 'Wrap-up', hint: 'Link it to your plan and stop.' },
];
...
export function InterviewRoom({
  ...
  // MVP uses the browser voice, which is free. Production uses pre-generated
  // cached audio: see docs/UNIT_ECONOMICS.md, never call TTS at runtime.
      ...
      <main className="mx-auto grid max-w-7xl gap-4 p-4 lg:grid-cols-[1fr_320px]">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
          <div className="mb-1 flex items-center gap-2">
            ...
            <button
              ...
              className="rounded-full px-2.5 py-1 text-[11px] font-semibold text-brand hover:bg-slate-100"
              style={{ color: institution.accent }}
            >
              Hear it again
            </button>
          </div>
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            {STRUCTURE.map((s) => (
              ...
              >
                <span className="mr-1.5 font-bold text-slate-400">{s.n}</span>
                {s.label}
              </span>
            ))}
          </div>
        ...
        <aside className="space-y-4">
          ...
          <MonitorPanel
            ...
            cameraOn={cameraOn}
            micOn={Boolean(streamRef.current?.getAudioTracks().length)}
          />
